...
task :version do
  ...
  # puts "The current released version of your pod is `#{spec.version}`"
  ...
  # puts "Releasing version `#{new_version_number}`"

  # puts "The current version is `#{spec.version}`, is that correct? [y/n]"
  ...
  # puts "You're about to release `#{new_version_number}`, is that correct? [y/n]"
  # exit if $stdin.gets.strip.downcase != 'y'
...
task :release do
  ...
  sh "git add -A && git commit -m 'Release #{spec_version}'"
